...
fn main() {
    ...
    for (cheat_start_index, cheat_start) in state_without_cheating.path.iter().enumerate() {
        let cheat_ends: Vec<(usize, &Vec2)> = state_without_cheating
            .path
            .iter()
            .enumerate()
            .skip(cheat_start_index)
            .filter(|(_, v)| {
                let distance = v.distance2(*cheat_start);
                (4..=400).contains(&distance)
            })
            .collect();

        for (cheat_end_index, cheat_end) in cheat_ends {
            let cheat = find_path(&grid, *cheat_start, *cheat_end, true, 21);
