...
fn explore(grid: &CharGrid, start: Vec2, end: Vec2) -> Option<(Vec<Vec2>, i32)> {
    let size = grid.len();
...
fn main() {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if explore(&grid, Vec2::new(0, 0), Vec2::new(size, size)).is_none() {
            println!("{line}");
            return;
        }
    }

    println!("Path is always available");
}
